<template>
  <div class="search-card">
    <div class="trip-types">
      <button type="button" :class="{ active: localForm.JourneyType === 1 }" @click="setJourneyType(1)">
        <span class="radio-dot"></span> One Way
      </button>
      <button type="button" :class="{ active: localForm.JourneyType === 2 }" @click="setJourneyType(2)">
        <span class="radio-dot"></span> Round Trip
      </button>
      <button type="button" disabled><span class="radio-dot"></span> Multi City</button>
    ...
    <div class="search-grid">
      <AirportSelector label="From" v-model:code="localForm.Origin" v-model:name="originName" placeholder="Dhaka" />
      <button class="swap-btn" type="button" @click="swapAirports">⇄</button>
      <AirportSelector label="To" v-model:code="localForm.Destination" v-model:name="destinationName" placeholder="Cox's Bazar" />
      <DateSelector label="Departure" v-model="localForm.DepartureDate" @change="emitChange" />
      <DateSelector v-if="localForm.JourneyType === 2" label="Return" v-model="localForm.ReturnDate" @change="emitChange" />
      <button class="search-btn" type="button" @click="$emit('search')">⌕</button>
    </div>
...
watch(() => props.form, value => Object.assign(localForm, value), { deep: true })

function emitChange() { emit('update:form', { ...localForm }) }
